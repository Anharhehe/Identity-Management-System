...
export default function FamilyMessagesPage() {
  ...
  const initSocket = useCallback((token: string) => {
    ...
    socket.on('connect', () => {
      setIsConnected(true);
      socket.emit('join_context', { context: 'family' });
    });
  ...
  const fetchConversations = async (token?: string) => {
    ...
    try {
      ...
      const [friendsRes, convsRes] = await Promise.all([
        fetch(`${API_BASE_URL}/friends/family`, {
          headers: { 'Authorization': `Bearer ${t}`, 'Content-Type': 'application/json' },
        }),
        fetch(`${API_BASE_URL}/messages/conversations/family`, {
          headers: { 'Authorization': `Bearer ${t}`, 'Content-Type': 'application/json' },
        }),
      ]);
      if (friendsRes.ok) {
  ...
  const fetchMessages = async (friendIdentityId: string) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/messages/family/${friendIdentityId}?limit=100`,
  ...
  return (
    ...
    >
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <div className="absolute -top-40 -left-20 w-[560px] h-[560px] rounded-full bg-amber-950/70 blur-[150px]" />
        <div className="absolute bottom-0 right-0 w-[420px] h-[420px] rounded-full bg-orange-950/50 blur-[130px]" />
      </div>

      <div className="relative z-10 h-full flex flex-col max-w-7xl mx-auto px-6 py-6">
        <div className="flex items-center justify-between mb-5 flex-shrink-0">
          <div>
            <p className="text-[11px] font-semibold text-amber-400 tracking-[0.22em] uppercase mb-1">
              Family Context
            </p>
            ...
            <div className="flex-1 overflow-y-auto">
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center h-40 gap-3 px-5 text-center">
                  ...
                  <p className="text-gray-600 text-xs leading-relaxed">No connections in family context yet.</p>
                </div>
              ) : (
                <div className="py-2">
                  {friends.map((friend) => {
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          borderLeft: isActive ? '2px solid rgba(217,119,6,0.7)' : '2px solid transparent',
                        }}
                        onMouseEnter={e => {
                      ...
                      >
                        <div
                          ...
                          style={{ background: isActive ? 'linear-gradient(135deg,#f59e0b,#d97706)' : 'rgba(217,119,6,0.2)' }}
                        >
                          {initials}
                        </div>
