...
export default function ProfessionalMessagesPage() {
  ...
  const initSocket = useCallback((token: string) => {
    ...
    socket.on('connect', () => {
      setIsConnected(true);
      socket.emit('join_context', { context: 'professional' });
    });

    socket.on('disconnect', () => setIsConnected(false));
    socket.on('connect_error', () => setIsConnected(false));

    // Incoming message from the OTHER user (own messages are added optimistically on send)
    socket.on('new_message', (msg: Message) => {
      ...
      if (activeFriend && msg.senderUserId !== myUserId && msg.senderIdentityId === activeFriend.id) {
        setMessages(prev => {
          ...
          return [...prev, msg]; // useEffect will smooth-scroll after DOM update
        });
      }

      // Update sidebar preview for incoming message
  ...
  // Scroll after every messages update (runs after DOM is committed)
  useEffect(() => {
  ...
  const fetchConversations = async (token?: string) => {
    ...
    try {
      ...
      const [friendsRes, convsRes] = await Promise.all([
        fetch(`${API_BASE_URL}/friends/professional`, {
          headers: { 'Authorization': `Bearer ${t}`, 'Content-Type': 'application/json' },
        }),
        fetch(`${API_BASE_URL}/messages/conversations/professional`, {
          headers: { 'Authorization': `Bearer ${t}`, 'Content-Type': 'application/json' },
        }),
      ]);
      if (friendsRes.ok) {
  ...
  const fetchMessages = async (friendIdentityId: string) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/messages/professional/${friendIdentityId}?limit=100`,
        { headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' } }
      );
      if (response.ok) {
        const data = await response.json();
        shouldScrollInstant.current = true; // instant jump when opening a chat
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Immediately show own message in chat
        setMessages(prev => [...prev, newMsg]);

        // Relay to recipient via socket
    ...
    >
      ...
      <div className="relative z-10 h-full flex flex-col max-w-7xl mx-auto px-6 py-6">
        ...
        <div className="flex items-center justify-between mb-5 flex-shrink-0">
          <div>
            <p className="text-[11px] font-semibold text-purple-400 tracking-[0.22em] uppercase mb-1">
            ...
            <div className="flex-1 overflow-y-auto">
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center h-40 gap-3 px-5 text-center">
                  ...
                  <p className="text-gray-600 text-xs leading-relaxed">No connections in professional context yet.</p>
                </div>
              ) : (
                <div className="py-2">
                  {friends.map((friend) => {
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          borderLeft: isActive ? '2px solid rgba(109,40,217,0.7)' : '2px solid transparent',
                        }}
                        onMouseEnter={e => {
                      ...
                      >
                        ...
                        <div
                          ...
                          style={{ background: isActive ? 'linear-gradient(135deg,#6d28d9,#7c3aed)' : 'rgba(109,40,217,0.2)' }}
                        >
                          {initials}
                        </div>
