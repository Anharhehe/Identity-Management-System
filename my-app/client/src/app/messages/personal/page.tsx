...
const Icons = {
  ...
  Heart: ({ className = 'w-3 h-3' }) => (
    <svg viewBox="0 0 24 24" fill="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" className={className}>
...
export default function PersonalMessagesPage() {
  ...
  const initSocket = useCallback((token: string) => {
    ...
    socket.on('connect', () => {
      setIsConnected(true);
      socket.emit('join_context', { context: 'personal' });
    });
  ...
  const fetchConversations = async (token?: string) => {
    try {
      ...
      // Fetch all friends
      const friendsResponse = await fetch(`${API_BASE_URL}/friends/personal`, {
        headers: {
      ...
      const conversationsResponse = await fetch(`${API_BASE_URL}/messages/conversations/personal`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
  ...
  const fetchMessages = async (friendIdentityId: string) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/messages/personal/${friendIdentityId}?limit=100`,
    ...
    >
      ...
      <div className="relative z-10 h-full flex flex-col max-w-7xl mx-auto px-6 py-6">
        <div className="flex items-center justify-between mb-5 flex-shrink-0">
          <div>
            <p className="text-[11px] font-semibold text-blue-400 tracking-[0.22em] uppercase mb-1">
              Personal Context
            </p>
            ...
            <div className="flex-1 overflow-y-auto">
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center h-40 gap-3 px-5 text-center">
                  ...
                  <p className="text-gray-600 text-xs leading-relaxed">No connections in personal context yet.</p>
                </div>
              ) : (
                <div className="py-2">
                  {friends.map((friend) => {
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          borderLeft: isActive ? '2px solid rgba(59,130,246,0.7)' : '2px solid transparent',
                        }}
                        onMouseEnter={e => {
                      ...
                      >
                        <div
                          ...
                          style={{ background: isActive ? 'linear-gradient(135deg,#3b82f6,#2563eb)' : 'rgba(59,130,246,0.2)' }}
                        >
                          {initials}
                        </div>
