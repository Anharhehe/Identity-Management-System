...
export default function PersonalMessagesPage() {
  ...
  const fetchMessages = async (friendIdentityId: string) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_BASE_URL}/messages/personal/${friendIdentityId}?limit=100`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
  ...
  return (
    <div className="min-h-screen dark:bg-gray-900 bg-white">
      <div className="max-w-6xl mx-auto px-4 py-4 h-screen flex flex-col">
        {/* Header */}
        <div className="mb-4">
          <h1 className="text-3xl font-bold dark:text-white text-gray-900">Personal Messages</h1>
        </div>
        ...
        <div className="flex gap-4 flex-1 overflow-hidden">
          ...
          <div className="w-72 dark:bg-gray-800 bg-gray-50 rounded-lg border dark:border-gray-700 border-gray-200 overflow-y-auto">
            {isLoading ? (
              ...
              <div className="p-4 text-center dark:text-gray-400 text-gray-600">No friends in personal context yet</div>
            ) : (
