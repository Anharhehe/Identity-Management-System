...
// ── Main Page ─────────────────────────────────────────────────────────────────
export default function OnlineFavoritesPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [friendsRes, requestsRes] = await Promise.all([
        fetch(`${apiUrl}/friends/online`, {
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        }),
        fetch(`${apiUrl}/friend-requests/online`, {
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        }),
      ]);
  ...
  const handleRemoveFriend = async (friendId: string) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/friends/remove`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({ friendIdentityId: friendId, context: 'online' }),
      });
  ...
  const handleAcceptRequest = async (requestId: string) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/friend-requests/accept`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({ requestId, context: 'online' }),
      });
        ...
        {friends.length === 0 ? (
          ...
          >
            ...
            <p className="text-gray-600 text-sm mb-7 max-w-xs leading-relaxed">
              Follow online profiles to build your network and see them here.
            </p>
            <Link
              href="/search/online"
              className="px-5 py-2.5 rounded-xl text-sm font-semibold text-emerald-300 cursor-pointer transition-all hover:text-emerald-200"
              style={{ background: 'rgba(16,185,129,0.12)', border: '1px solid rgba(16,185,129,0.25)' }}
            >
              Explore Online Profiles
            </Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {friends.map((friend) => (
              <FriendCard
                ...
                onViewProfile={() => router.push(`/search/online/${friend.id}`)}
...
function FriendCard({
  ...
  return (
    <div
      ...
      style={{
        background: hovered ? 'rgba(16,185,129,0.06)' : 'rgba(255,255,255,0.025)',
        border: hovered ? '1px solid rgba(16,185,129,0.28)' : '1px solid rgba(255,255,255,0.06)',
      }}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded-full text-[11px] font-medium border bg-emerald-500/15 text-emerald-300 border-emerald-500/25">
              <Icons.GlobeIcon className="w-3 h-3" />
              Online
            </span>
          </div>
        </div>
