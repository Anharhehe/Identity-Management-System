  ...
  const fetchData = async () => {
    try {
      ...
      const [friendsRes, requestsRes] = await Promise.all([
        fetch(`${apiUrl}/friends/personal`, {
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        }),
        fetch(`${apiUrl}/friend-requests/personal`, {
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        }),
      ]);
  ...
  const handleRemoveFriend = async (friendId: string) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/friends/remove`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({ friendIdentityId: friendId, context: 'personal' }),
      });
  ...
  const handleAcceptRequest = async (requestId: string) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/friend-requests/accept`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({ requestId, context: 'personal' }),
      });
    ...
    <div className="min-h-screen bg-[#09090f]" style={{ fontFamily: "'Sora', 'DM Sans', sans-serif" }}>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="flex items-end justify-between mb-10">
          <div>
            <p className="text-[11px] font-semibold text-sky-400 tracking-[0.22em] uppercase mb-2">
              Personal Context
            </p>
        ...
        {friends.length === 0 ? (
          ...
          >
            ...
            <p className="text-gray-600 text-sm mb-7 max-w-xs leading-relaxed">
              Follow personal profiles to build your network and see them here.
            </p>
            <Link
              href="/search/personal"
              className="px-5 py-2.5 rounded-xl text-sm font-semibold text-sky-300 cursor-pointer transition-all hover:text-sky-200"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {friends.map((friend) => (
              <FriendCard
                ...
                onViewProfile={() => router.push(`/search/personal/${friend.id}`)}
...
function FriendCard({
  ...
  return (
    <div
      ...
      style={{
        background: hovered ? 'rgba(14,165,233,0.06)' : 'rgba(255,255,255,0.025)',
        border: hovered ? '1px solid rgba(14,165,233,0.28)' : '1px solid rgba(255,255,255,0.06)',
      }}
    ...
    >
      ...
      <div className="flex gap-2 mt-auto">
        <button
          ...
          onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = 'rgba(14,165,233,0.22)'; }}
