  ...
  const fetchData = async () => {
    try {
      ...
      const [friendsRes, requestsRes] = await Promise.all([
        fetch(`${apiUrl}/friends/professional`, {
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        }),
        fetch(`${apiUrl}/friend-requests/professional`, {
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        }),
      ]);
  ...
  const handleRemoveFriend = async (friendId: string) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/friends/remove`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({ friendIdentityId: friendId, context: 'professional' }),
      });
  ...
  const handleAcceptRequest = async (requestId: string) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/friend-requests/accept`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({ requestId, context: 'professional' }),
      });
        ...
        {friends.length === 0 ? (
          ...
          >
            ...
            <p className="text-gray-600 text-sm mb-7 max-w-xs leading-relaxed">
              Follow professional profiles to build your network and see them here.
            </p>
            <Link
              href="/search/professional"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {friends.map((friend) => (
              <FriendCard
                ...
                onViewProfile={() => router.push(`/search/professional/${friend.id}`)}
