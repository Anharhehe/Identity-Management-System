...
const Icons = {
  ...
  GlobeIcon: ({ className = 'w-3 h-3' }: { className?: string } = {}) => (
...
export default function OnlineProfilePage() {
  ...
  const checkFriendshipStatus = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/friends/check/${identityId}/online`, {
  ...
  const handleFollow = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/friends/add`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({ friendIdentityId: identityId, context: 'online' }),
      });
  ...
  const handleSendRequest = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/friend-requests/send`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({ recipientIdentityId: identityId, context: 'online' }),
      });
  ...
  const handleMessage = () => router.push(`/messages/online`);
    ...
    return (
      <div className="min-h-screen bg-[#09090f] flex items-center justify-center px-6">
        <div className="rounded-2xl p-10 text-center max-w-sm w-full" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)' }}>
          ...
          <button onClick={() => router.back()} className="flex items-center gap-2 mx-auto px-5 py-2.5 rounded-xl text-sm font-semibold text-white cursor-pointer" style={{ background: 'linear-gradient(135deg,#059669,#10b981)', border: '1px solid rgba(16,185,129,0.4)' }}>
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-6 py-12">
        ...
        <div className="rounded-2xl p-8 mb-5" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)' }}>
          <div className="flex flex-col items-center text-center mb-8">
            <div className="w-20 h-20 rounded-2xl flex items-center justify-center text-2xl font-bold text-white mb-5 shadow-xl shadow-emerald-950/50" style={{ background: 'linear-gradient(135deg,#059669,#10b981)' }}>
              {initials}
            </div>
            <h1 className="text-3xl font-bold text-white tracking-tight mb-1">{identity.preferredName}</h1>
            <div className="flex items-center gap-2 mt-2 flex-wrap justify-center">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border bg-emerald-500/15 text-emerald-300 border-emerald-500/25">
                <Icons.GlobeIcon className="w-3 h-3" /> Online
              </span>
              ...
              {isFriend && (
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border bg-emerald-500/10 text-emerald-400 border-emerald-500/20">
...
function ActionButton({ onClick, icon, label, disabled, loading, variant }: {
  ...
  const styles: Record<ButtonVariant, React.CSSProperties> = {
    primary: {
      ...
      boxShadow: hovered ? '0 6px 24px rgba(16,185,129,0.35)' : '0 4px 16px rgba(16,185,129,0.25)',
    },
    secondary: {
