...
export default function OnlineSearchPage() {
  ...
  const fetchPublicIdentities = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/identities/public/online`, {
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api';
      const response = await fetch(`${apiUrl}/identities/all/online`, {
        headers: { 'Content-Type': 'application/json' },
      ...
      setFilteredIdentities(
        (data.identities || []).filter((i: Identity) =>
  ...
  return (
    <div className="min-h-screen bg-[#09090f]" style={{ fontFamily: "'Sora', 'DM Sans', sans-serif" }}>
      ...
      {/* Ambient glows — emerald/teal for online context */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <div className="absolute -top-40 -left-20 w-[560px] h-[560px] rounded-full bg-emerald-950/60 blur-[150px]" />
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 py-12">
        ...
        {/* ── Page heading ── */}
        <div className="mb-10">
          <p className="text-[11px] font-semibold text-emerald-400 tracking-[0.22em] uppercase mb-2">
            Directory
          </p>
          ...
          <p className="text-gray-600 text-sm mt-3">
            Discover and connect with online identity profiles.
          </p>
        </div>
          ...
          >
            ...
            <p className="text-gray-600 text-sm max-w-xs leading-relaxed">
              {searchQuery
                ...
                : 'Public online profiles will appear here once created.'}
            </p>
            {searchQuery && (
              <button
                onClick={clearSearch}
                className="mt-6 px-5 py-2 rounded-xl text-sm font-medium text-emerald-300 cursor-pointer transition-all"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredIdentities.map((identity) => (
              <ProfileCard
                key={identity.id}
                identity={identity}
                onClick={() => router.push(`/search/online/${identity.id}`)}
              />
            ))}
          </div>
        )}
...
function ProfileCard({ identity, onClick }: { identity: Identity; onClick: () => void }) {
  ...
  return (
    <button
      ...
      style={{
        background: hovered ? 'rgba(16,185,129,0.07)' : 'rgba(255,255,255,0.025)',
        border: hovered ? '1px solid rgba(16,185,129,0.3)' : '1px solid rgba(255,255,255,0.06)',
        transform: hovered ? 'translateY(-2px)' : 'translateY(0)',
        boxShadow: hovered ? '0 12px 40px rgba(16,185,129,0.12)' : 'none',
      }}
    ...
    >
      ...
      <div className="flex items-start justify-between mb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded-full text-[11px] font-medium border bg-emerald-500/15 text-emerald-300 border-emerald-500/25">
              <Icons.Globe className="w-3 h-3" /> Online
            </span>
          </div>
        </div>
        ...
        >
          {isPrivate ? <><Icons.Lock /> Private</> : <><Icons.Unlock /> Public</>}
        </span>
      </div>
      ...
      >
        ...
        <span
          ...
          style={{
            background: hovered ? 'rgba(16,185,129,0.25)' : 'rgba(16,185,129,0.1)',
