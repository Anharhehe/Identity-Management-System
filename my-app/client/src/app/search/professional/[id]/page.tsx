...
export default function ProfessionalProfilePage() {
  ...
  const handleFollow = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/friends/add`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({ friendIdentityId: identityId, context: 'professional' }),
      });
  ...
  const handleSendRequest = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/friend-requests/send`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({ recipientIdentityId: identityId, context: 'professional' }),
      });
  ...
  const handleMessage = () => router.push(`/messages/professional`);
  ...
  if (error || !identity) {
    return (
      <div className="min-h-screen bg-[#09090f] flex items-center justify-center px-6">
        ...
        >
          <span className="text-red-400 flex justify-center mb-4"><Icons.AlertCircle className="w-8 h-8" /></span>
          ...
          <button
            ...
            style={{ background: 'linear-gradient(135deg,#6d28d9,#7c3aed)', border: '1px solid rgba(109,40,217,0.4)' }}
          >
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-6 py-12">
        ...
        >
          ...
          <div className="flex flex-col items-center text-center mb-8">
            <div
              className="w-20 h-20 rounded-2xl flex items-center justify-center text-2xl font-bold text-white mb-5 shadow-xl shadow-purple-950/50"
            ...
            <div className="flex items-center gap-2 mt-2 flex-wrap justify-center">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border bg-violet-500/15 text-violet-300 border-violet-500/25">
                <Icons.Briefcase className="w-3 h-3" /> Professional
              </span>
              <span
            ...
            <>
              ...
              >
                ...
                <p className="text-gray-500 text-sm leading-relaxed">
                  This is a private profile. Send a friend request to see full details.
                </p>
              </div>
            </>
          )}
...
function ActionButton({
  ...
  const styles: Record<ButtonVariant, React.CSSProperties> = {
    primary: {
      ...
      color: '#fff',
      boxShadow: hovered ? '0 6px 24px rgba(109,40,217,0.35)' : '0 4px 16px rgba(109,40,217,0.25)',
    },
    secondary: {
