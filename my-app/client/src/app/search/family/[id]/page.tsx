...
export default function FamilyProfilePage() {
  ...
  const checkFriendshipStatus = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/friends/check/${identityId}/family`, {
  ...
  const handleFollow = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/friends/add`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({ friendIdentityId: identityId, context: 'family' }),
      });
  ...
  const handleSendRequest = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/friend-requests/send`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({ recipientIdentityId: identityId, context: 'family' }),
      });
  ...
  const handleMessage = () => router.push('/messages/family');
    ...
    return (
      <div className="min-h-screen bg-[#09090f] flex items-center justify-center px-6">
        ...
        >
          ...
          <button
            onClick={() => router.back()}
            className="flex items-center gap-2 mx-auto px-5 py-2.5 rounded-xl text-sm font-semibold text-white cursor-pointer transition-all"
            style={{
  ...
  return (
    <div className="min-h-screen bg-[#09090f]" style={{ fontFamily: "'Sora', 'DM Sans', sans-serif" }}>
      ...
      {/* Ambient glows — rose/pink tones for family context */}
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-6 py-12">
        ...
        <button
          onClick={() => router.back()}
          className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-300 transition-colors cursor-pointer mb-8 group"
        >
        ...
        >
          ...
          <div className="flex flex-col items-center text-center mb-8">
            <div
              className="w-20 h-20 rounded-2xl flex items-center justify-center text-2xl font-bold text-white mb-5 shadow-xl shadow-rose-950/50"
            ...
            <div className="flex items-center gap-2 mt-2 flex-wrap justify-center">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border bg-rose-500/15 text-rose-300 border-rose-500/25">
                <Icons.Family className="w-3 h-3" /> Family
              </span>
              <span
              ...
              {isFriend && (
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border bg-rose-500/10 text-rose-400 border-rose-500/20">
          ...
          <div className="space-y-4">
            ...
            {showFullProfile && identity.nickname && (
              <InfoRow label="Nickname" value={identity.nickname} />
            )}

            {identity.createdAt && (
              <div>
                <p className="text-[10px] text-gray-600 uppercase tracking-[0.15em] font-medium mb-0.5">
                  Member Since
                </p>
            ...
            <>
              ...
              >
                ...
                <p className="text-gray-500 text-sm leading-relaxed">
                  This is a private profile. Send a connection request to see full details.
                </p>
              </div>
            </>
          )}
...
function ActionButton({
  ...
  const styles: Record<ButtonVariant, React.CSSProperties> = {
    primary: {
      ...
      color: '#fff',
      boxShadow: hovered
        ? '0 6px 24px rgba(244,63,94,0.35)'
        : '0 4px 16px rgba(244,63,94,0.25)',
    },
    secondary: {
      background: hovered ? 'rgba(244,63,94,0.15)' : 'rgba(244,63,94,0.08)',
