...
export default function FamilySearchPage() {
  ...
  const fetchPublicIdentities = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/identities/public/family`, { method: 'GET', headers: { 'Content-Type': 'application/json' } });
      ...
      setIdentities([]); setFilteredIdentities([]);
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api';
      const response = await fetch(`${apiUrl}/identities/all/family`, { method: 'GET', headers: { 'Content-Type': 'application/json' }, signal: controller.signal });
      if (!response.ok) throw new Error('Failed to fetch identities');
      const data = await response.json();
      const q = query.toLowerCase();
      setFilteredIdentities((data.identities || []).filter((i: Identity) =>
        i.preferredName.toLowerCase().includes(q) || i.legalName.toLowerCase().includes(q) || i.nickname.toLowerCase().includes(q)
      ));
    } catch (err: any) {
  ...
  const clearSearch = () => {
    if (searchAbortRef.current) searchAbortRef.current.abort();
    setSearchQuery(''); setFilteredIdentities(identities);
  };

  return (
    <div className="min-h-screen bg-[#09090f]" style={{ fontFamily: "'Sora', 'DM Sans', sans-serif" }}>
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <div className="absolute -top-40 -left-20 w-[560px] h-[560px] rounded-full bg-rose-950/60 blur-[150px]" />
        <div className="absolute bottom-0 right-0 w-[420px] h-[420px] rounded-full bg-pink-950/50 blur-[130px]" />
      </div>
      <div className="relative z-10 max-w-6xl mx-auto px-6 py-12">
        <div className="mb-10">
          <p className="text-[11px] font-semibold text-rose-400 tracking-[0.22em] uppercase mb-2">Directory</p>
          <p className="text-gray-600 text-sm mt-3">Discover and connect with family identity profiles.</p>
        </div>
        <style>{`
        ...
        <div className="relative mb-10">
          <div
            ...
            onFocusCapture={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = 'rgba(244,63,94,0.5)'; el.style.background = 'rgba(244,63,94,0.06)'; }}
          ...
          >
            ...
            <input type="text" placeholder="Search by preferred name, legal name, or nickname…" value={searchQuery} onChange={(e) => handleSearch(e.target.value)} autoComplete="off" suppressHydrationWarning
            ...
            />
            {searchQuery && (<button onClick={clearSearch} className="flex-shrink-0 text-gray-600 hover:text-gray-300 transition-colors cursor-pointer"><Icons.X /></button>)}
          </div>
        </div>
        {isLoading ? (
          ...
          <div className="rounded-2xl py-24 flex flex-col items-center justify-center text-center" style={{ border: '1px dashed rgba(255,255,255,0.07)' }}>
            ...
            <p className="text-gray-600 text-sm max-w-xs leading-relaxed">
              {searchQuery ? `No profiles match "${searchQuery}". Try a different search term.` : 'Public family profiles will appear here once created.'}
            </p>
            {searchQuery && (<button onClick={clearSearch} className="mt-6 px-5 py-2 rounded-xl text-sm font-medium text-rose-300 cursor-pointer" style={{ background: 'rgba(244,63,94,0.12)', border: '1px solid rgba(244,63,94,0.22)' }}>Clear search</button>)}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredIdentities.map((identity) => (<ProfileCard key={identity.id} identity={identity} onClick={() => router.push(`/search/family/${identity.id}`)} />))}
          </div>
        )}
      </div>
    </div>
...
function ProfileCard({ identity, onClick }: { identity: Identity; onClick: () => void }) {
  ...
  return (
    <button onClick={onClick} className="text-left w-full rounded-2xl p-6 flex flex-col transition-all duration-200 cursor-pointer"
      style={{ background: hovered ? 'rgba(244,63,94,0.07)' : 'rgba(255,255,255,0.025)', border: hovered ? '1px solid rgba(244,63,94,0.3)' : '1px solid rgba(255,255,255,0.06)', transform: hovered ? 'translateY(-2px)' : 'translateY(0)', boxShadow: hovered ? '0 12px 40px rgba(244,63,94,0.12)' : 'none' }}
      onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
    >
      <div className="flex items-start justify-between mb-5">
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 font-bold text-sm text-white" style={{ background: 'linear-gradient(135deg,#be123c,#f43f5e)' }}>{initials}</div>
          <div>
            ...
            <span className="inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded-full text-[11px] font-medium border bg-rose-500/15 text-rose-300 border-rose-500/25">
              <Icons.FamilyIcon className="w-3 h-3" /> Family
            </span>
          </div>
        </div>
        <span className={`flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-full border ${isPrivate ? 'bg-white/5 text-gray-400 border-white/10' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'}`}>
      ...
      <div className="flex-1 space-y-3 mb-5">
        ...
        {!isPrivate && identity.nickname && (<div><p className="text-[10px] text-gray-600 uppercase tracking-[0.15em] mb-0.5 font-medium">Nickname</p><p className="text-sm font-medium text-gray-200">{identity.nickname}</p></div>)}
        {isPrivate && (<div className="flex items-center gap-2 text-gray-600 text-xs"><Icons.Lock className="w-3 h-3" /><span>Profile details are private</span></div>)}
      </div>
      <div className="flex items-center justify-between pt-4" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
        ...
        <span className="flex items-center justify-center w-7 h-7 rounded-lg" style={{ background: hovered ? 'rgba(244,63,94,0.25)' : 'rgba(244,63,94,0.1)', border: '1px solid rgba(244,63,94,0.2)', color: '#fda4af' }}>
